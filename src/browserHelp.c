...
\*************************************************************************/
/* Help package that calls Netscape */
  ...
  /*   or "quit" to terminate the browser           */
{
  ...
  /*   (Use -id for speed) */
	...
	sprintf(command,"%s -id 0x%x -remote 'openURL(%s%s)' &",
	  NETSCAPEPATH,(unsigned)netscapew,url,bookmark);
#else
        sprintf(command,"%s -id 0x%x -remote \"openURL(%s%s)\"",
	  NETSCAPEPATH,(unsigned)netscapew,url,bookmark);
#endif
...
#ifndef VMS
	sprintf(command,"%s -id 0x%x -remote 'openURL(%s%s)' &",
	  envstring,(unsigned)netscapew,url,bookmark);
#else
	sprintf(command,"%s -id 0x%x -remote \"openURL(%s%s)\" &",
  ...
  /* Does nothing with "quit"                  */
{
   ...
   *   It may have trouble if the URL has spaces */
	...
	print("callBrowser: Cannot start browser:\n"
	  "%s %s\n"
